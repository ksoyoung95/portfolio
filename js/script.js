...
function initCareerModal() {
  ...
  qs("#careerGrid")?.addEventListener("click", (e) => {
    ...
    qs("#careerTitle").textContent = c.company;
    qs("#careerTag").textContent = c.tag;
    qs("#careerPeriod").textContent = c.period;
    qs("#careerTasks").innerHTML = (c.tasks || []).map(t => `<li>${t}</li>`).join("");
...
function updateWorkImage() {
  ...
  const imgs = currentWork.popupImages || [];
...
function initPortfolio() {
  ...
  qsa(".tab").forEach(btn => {
    btn.addEventListener("click", () => {
      qsa(".tab").forEach(t => t.classList.remove("isActive"));
